...
class GameListViewModel(
    ...
    private val _selectedGameIdForSheet = MutableStateFlow<Int?>(null)
    ...
    fun toggleGroupCollapsed(groupName: String) {
        if (_collapsedGroups.value.contains(groupName)) {
            _collapsedGroups.value -= groupName
        } else {
            _collapsedGroups.value += groupName
        }
    }

    fun showGameDetailSheet(gameId: Int) {
        _selectedGameIdForSheet.value = gameId
    }

    fun dismissGameDetailSheet() {
        _selectedGameIdForSheet.value = null
    }
